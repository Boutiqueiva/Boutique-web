...
function loadProductData(productId) {
    ...
    product.imagenes.forEach((imgSrc, index) => {
        ...
        // Corregir ruta de imágenes para GitHub Pages
        const correctedImgSrc = correctImagePath(imgSrc);
        imgElement.src = correctedImgSrc;
        imgElement.alt = `${product.nombre} - Vista ${index + 1}`;
        imgElement.addEventListener('click', () => {
            updateMainImage(correctedImgSrc, index);
    ...
    if (product.imagenes.length > 0) {
        // Corregir ruta de la imagen principal
        const correctedMainImg = correctImagePath(product.imagenes[0]);
        document.getElementById('mainProductImage').src = correctedMainImg;
    ...
    if (product.tallas && product.tallas.length > 0) {
        ...
        // Seleccionar primera talla por defecto si hay tallas
        setTimeout(() => {
            const firstSizeBtn = sizeButtonsContainer.querySelector('button');
            if (firstSizeBtn) {
                firstSizeBtn.click();
            }
        }, 100);
        ...
        // Si no hay tallas, activar el botón de reserva inmediatamente
        selectedSize = "Única";
        setTimeout(updateReserveButton, 100);
    }

    initImageNavigation(product.imagenes);
}

// Función para corregir rutas de imágenes para GitHub Pages
function correctImagePath(imgPath) {
    // Si la imagen ya es una URL completa, déjala como está
    if (imgPath.startsWith('http://') || imgPath.startsWith('https://')) {
        return imgPath;
    }
    
    // Si la imagen comienza con 'src/', conviértela a ruta relativa
    if (imgPath.startsWith('src/')) {
        // Para GitHub Pages, usa la ruta desde la raíz del sitio
        return './' + imgPath; // o simplemente imgPath si tu estructura lo requiere
    }
    
    // Si no tiene prefijo, asume que es relativa
    return imgPath;
}
...
function initImageNavigation(images) {
    ...
    prevBtn.addEventListener('click', () => {
        if (images.length > 0) {
            ...
            const correctedImg = correctImagePath(images[currentImageIndex]);
            updateMainImage(correctedImg, currentImageIndex);
...
// ============================================================
// FUNCIONES PARA EL BOTÓN DE APARTAR WHATSAPP
// ============================================================
...
function initReserveButton() {
    ...
    console.log('Botón de reserva encontrado:', reserveBtn);
    ...
    reserveBtn.addEventListener('click', function(e) {
        ...
        if (!selectedSize && currentProduct.tallas && currentProduct.tallas.length > 0) {
...
function updateReserveButton() {
    ...
    if (!reserveBtn) {
        console.error('Botón de reserva no encontrado en updateReserveButton');
        return;
    }
    
    console.log('Actualizando botón, selectedSize:', selectedSize);
    
    // Si el producto tiene tallas y no hay ninguna seleccionada, deshabilitar botón
...
function generateWhatsAppLink() {
    ...
    const productPrice = currentProduct.precio;
    const productCategory = currentProduct.categoria;
    const productDescription = currentProduct.descripcion.substring(0, 100) + '...';
    const sizeText = selectedSize ? `Talla: ${selectedSize}` : 'Talla única';
    const productLink = window.location.href;
    
    // Crear mensaje completo con emojis compatibles
    // Usamos emojis básicos que funcionan en todos los dispositivos
    const message = `¡Hola! 😊
...
¡Gracias!`;
    ...
    // Crear enlace de WhatsApp con codificación adecuada
...
function initAddToCartButton() {
    ...
    addToCartBtn.addEventListener('click', () => {
        ...
        alert(`¡${productName} ha sido agregado a tu bolsa de compra!`);
    });
}
...
function initPage() {
    ...
    if (productId) {
        ...
        initFavoriteButton();
        initAddToCartButton();
        initReserveButton(); // Nueva función agregada
    } else {
...
// Agregar estilo CSS para animación de pulso
...
style.textContent = `
    ...
    /* Estilos para imágenes en GitHub Pages */
    .thumbnail-gallery img, #mainProductImage {
        max-width: 100%;
        height: auto;
        display: block;
    }
